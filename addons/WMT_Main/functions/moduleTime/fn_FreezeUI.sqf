...
if (!isnil "wmt_feature_frzLegacyActions") then {
    if (leader player == player || serverCommandAvailable('#kick') ) then {
        _action = player addAction ["<t color='#ffa31f'>"+localize('STR_WMT_DontRemove')+"</t>",{cursorTarget setVariable ["PlayerName", "AI", true];}, [], -1, false, true, "", 'cursorTarget isKindOf "Man" && {cursorTarget in (units group player)} && {(cursorTarget getVariable ["PlayerName",""]) == "" } '];
        if(isNil "WMT_flg_NotAllowGearActionWhileFreeze") then {
            _gearaction = player addAction ["<t color='#ffa31f'>"+localize('STR_WMT_FreezeAIINv')+"</t>", {player action ["Gear", cursorTarget];},  [], -1, false, true, "", 'cursorTarget isKindOf "Man" && {cursorTarget in (units group player)} && {!isplayer cursorTarget}'];
            };
    };
};
...
while {WMT_pub_frzState < 3} do {
    ...
    PR(_infoStr1) = format ["<t size='0.85' color='%2'>%1</t>", [0 max (_sec min _time3),"MM:SS"] call BIS_fnc_secondsToString, if (_sec >= 30 ) then {"#cccccc"} else {"#ff0000"}];
